...
def update_elkan(
    ...
    inner_sums_mixed, inner_sums = _calc_inner_sums_mixed(kernel_matrix, labels, labels_old, n_clusters, return_new_sums=True)
...
def _calc_center_dists(double[::1] inner_sums_new, double[::1] inner_sums_mixed, double[::1] inner_sums_old, long[::1] sizes_new, long[::1] sizes_old):
    cdef:
        Py_ssize_t i
...
def _calc_inner_sums_mixed(double[:, ::1] kernel_matrix, long[::1] labels, long[::1] labels_old, long n_clusters, return_new_sums=True):
    cdef:
...
cdef double _calc_outer_sum_single(double[:, ::1] kernel_matrix, long elem_index, long cluster_index, long[::1] labels) nogil:
    cdef:
        double outer_sum = 0.
        Py_ssize_t i
