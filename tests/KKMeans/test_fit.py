...
def test_with_random_centers_low_nclusters(
    n_samples, n_features, n_clusters, seed, alg
):
    """
    ...
    kkmeans = KKMeans(
        n_clusters, n_init=1, init=centers, rng=seed, algorithm=alg, tol=0
    )
    kmeans.fit(data),
    kkmeans.fit(data)
...
def test_with_centers_empty_clusters(n_samples, n_features, n_clusters, seed, alg):
    ...
    kkmeans = KKMeans(
        n_clusters, n_init=1, init=centers, rng=seed, algorithm=alg, tol=0, max_iter=100
    )
    kkmeans.fit(data)
