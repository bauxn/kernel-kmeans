...
def test_sigmoid_kernel(max_data, xsize, ysize, dim, c_0, gamma):
    ...
    Y = RNG.random((ysize, dim)) * max_data
    assert np.allclose(
        ctrl.sigmoid_kernel(X, Y, gamma, c_0), 
        sigmoid_kernel(X, Y, gamma, c_0))
...
def test_laplacian_kernel(max_data, xsize, ysize, dim, gamma):
    ...
    Y = RNG.random((ysize, dim)) * max_data
    assert np.allclose(
        ctrl.laplacian_kernel(X, Y, gamma), 
        laplacian_kernel(X, Y, gamma))