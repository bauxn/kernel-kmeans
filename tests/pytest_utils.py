...
def build_starting_distance(kernel_matrix, n_clusters):
    ...
    return np.ascontiguousarray(np.tile(np.diag(kernel_matrix), (n_clusters, 1)).T, dtype=np.double)

def create_labels(sizes, shuffled=True):
    labels = np.concatenate([np.asarray([i] * sizes[i], dtype=np.int_) for i in range(len(sizes))])
    if shuffled:
        RNG.shuffle(labels)
    return labels
